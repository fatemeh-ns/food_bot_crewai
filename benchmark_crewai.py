...
INSTRUCTIONS = """You are a food ordering assistant.
...
agent_with_tools = Agent(
    role="Food Agent With Tools",
    goal="Help users find and order food based on their preferences and budget. Always use the available tools when the user asks for recommendations or orders.",
...
agent_no_tools = Agent(
    role="Food Agent No Tools",
    goal="Help users find and order food based on their preferences and budget without using tools.",
    backstory=INSTRUCTIONS,
    tools=[],
    llm=llm,
    verbose=False
...
WITH_TOOLS_TEST_INPUT = (
    f"{BASE_TEST_INPUT}\n"
    "برای پاسخ دادن حتما از ابزارهای recommend_food و reserve_food استفاده کن. "
    "ابتدا با recommend_food غذاهای مناسب را پیدا کن و اگر سفارش لازم بود از reserve_food استفاده کن."
)
...
def setup_database() -> tuple[sqlite3.Connection, Path]:
    ...
    """)
    for table_name in ["benchmark_sessions", "benchmark_averages"]:
        columns = [row["name"]
                   for row in conn.execute(f"PRAGMA table_info({table_name})")]
        for column_name in ["base_input", "with_tools_input", "without_tools_input"]:
...
def calculate_summary(conn: sqlite3.Connection, session_id: int, label: str) -> sqlite3.Row:
    return conn.execute("""
        SELECT
        ...
        FROM benchmark_runs
        WHERE session_id = ? AND agent_label = ?
    """, (session_id, label)).fetchone()


def get_run_times(conn: sqlite3.Connection, session_id: int, label: str) -> list[sqlite3.Row]:
    return conn.execute("""
        SELECT run_number, elapsed_seconds
        FROM benchmark_runs
        WHERE session_id = ? AND agent_label = ?
        ORDER BY run_number
    """, (session_id, label)).fetchall()
...
def run_benchmark(agent: Agent, label: str, mode: str, test_input: str, conn: sqlite3.Connection, session_id: int):
    print(f"\n{'='*55}")
    print(f"  {label}")
    print(f"{'='*55}")
    ...
    for i in range(NUM_RUNS):
        task = Task(
            description=f"""The user says: "{test_input}"
...
If tools are available and the prompt asks for tools, use the recommend_food tool to find matching food items and use reserve_food only if an order is requested.
Return a helpful response to the user in the same language as the user.""",
            expected_output="A helpful response with food recommendations or order confirmation",
            agent=agent
        )
        ...
        save_run_result(conn, session_id, label, i +
                        1, test_input, output, elapsed)

    summary = calculate_summary(conn, session_id, label)
    run_times = get_run_times(conn, session_id, label)
    ...
    for run_time in run_times:
        print(
            f"  Run {run_time['run_number']}: {run_time['elapsed_seconds']}s")
...
def main():
    ...
    try:
        print("\nStarting CrewAI Food Agent benchmark")
